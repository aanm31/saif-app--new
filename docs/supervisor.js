...
openHhArena = function(){if(!isSupervisor()||currentUser?.role==="owner")return originalOpenHhArena();const match=hhArena,type=match.matchType==="official"?"مباراة رسمية":"مباراة ودية";openModal(`<div class="hh-arena"><h2>${match.icon} ${escapeHtml(match.gameName)}</h2><div class="hh-arena-note"><strong>${type}</strong> · ${match.matchType==="official"?"تُضاف نقاط الفائزين إلى الرصيد الرئيسي بعد الاعتماد.":"لا تُحتسب أي نقاط للمشاركين."}</div><div class="hh-scoreboard">${match.sides.map(side=>`<div class="hh-score"><div><strong>${escapeHtml(side.name)}</strong><small>${escapeHtml(side.members.join("، "))}</small></div><b id="hhScore-${side.key}">0</b><button onclick="addHhScore('${side.key}')">+</button></div>`).join("")}</div></div><div class="modal-actions"><button class="outline-button" onclick="closeModal()">حفظ والمتابعة لاحقًا</button><button class="primary-button" onclick="finishHhMatch()">اعتماد النتائج</button></div>`)};
